const express = require('express');
const multer = require('multer');
const cors = require('cors');
const fs = require('fs');
const path = require('path');

const app = express();
const PORT = process.env.PORT || 3300;

// Middleware
app.use(cors());

// Configure multer for file uploads
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadDir = 'uploads';
    if (!fs.existsSync(uploadDir)) {
      fs.mkdirSync(uploadDir);
    }
    cb(null, uploadDir);
  },
  filename: function (req, file, cb) {
    // Keep original filename with timestamp
    const timestamp = Date.now();
    const ext = path.extname(file.originalname);
    const name = path.basename(file.originalname, ext);
    cb(null, `${name}_${timestamp}${ext}`);
  }
});

const upload = multer({
  storage: storage,
  limits: {
    fileSize: 10 * 1024 * 1024 // 10MB limit
  },
  fileFilter: function (req, file, cb) {
    // Only accept JSON files
    if (file.mimetype === 'application/json' ||
        path.extname(file.originalname).toLowerCase() === '.json') {
      cb(null, true);
    } else {
      cb(new Error('Only JSON files are allowed!'), false);
    }
  }
});

// Handle multipart form data with JSON file upload
app.post('/upload', upload.single('jsonFile'), (req, res) => {
  try {
    console.log('Received form data:', req.body);
    console.log('Received file:', req.file);

    if (!req.file) {
      return res.status(400).json({
        success: false,
        message: 'No JSON file uploaded'
      });
    }

    // Read and parse the JSON file
    const fileContent = fs.readFileSync(req.file.path, 'utf8');
    const jsonContent = JSON.parse(fileContent);

    res.json({
      success: true,
      message: 'JSON file received and processed successfully',
      formData: req.body,
      file: {
        originalName: req.file.originalname,
        filename: req.file.filename,
        size: req.file.size,
        content: jsonContent
      },
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('Error processing JSON file:', error);
    res.status(400).json({
      success: false,
      message: 'Error processing JSON file',
      error: error.message
    });
  }
});

// Start server
app.listen(PORT, () => {
  console.log(`FormData server is running on port ${PORT}`);
  console.log(`Endpoint: POST http://localhost:${PORT}/upload`);
  console.log(`Accepts multipart/form-data with a JSON file field named 'jsonFile'`);
});
